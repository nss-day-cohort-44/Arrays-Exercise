...
export const getStudentsByInstructorId = (instructorId) => {
  return students.filter((s) => s.instructorId === instructorId);
};
...
export const getPolyglotStudents = (languageCount) => {
  return students.filter((s) => s.languages.length >= languageCount);
};
...
export const getStudentsByLanguages = (languages) =>
  students.filter((student) => {
    return languages.every((lang) => student.languages.includes(lang));
  });
